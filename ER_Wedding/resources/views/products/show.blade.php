...
<div class="bg-white p-8 rounded-lg shadow-xl max-w-4xl mx-auto mt-8">
    @if($product->image)
        <img src="{{ asset('storage/products/' . $product->image) }}"
             alt="{{ $product->name }}"
             class="rounded-lg mb-6 w-full h-auto mx-auto"
    ...
    <div class="mb-4">
        ...
        <p class="text-sm text-gray-500">{{ $product->created_at->timezone('Asia/Jakarta')->format('d M Y, H:i') }}</p>
    </div>

    <p class="mt-4 text-lg text-gray-700 leading-relaxed">{{ $product->description }}</p>
</div>
